...
function useAnimation(
  ...
  options: { fps: number; autoplay: boolean; loop: boolean; onFrame?: (index: number) => void }
      ...
      if (accumulatorRef.current >= frameInterval) {
        ...
        setFrameIndex((prev) => {
          ...
          if (next >= frames.length) {
            if (options.loop) { options.onFrame?.(0); return 0; }
  ...
  for (let frame = 0; frame < 16; frame++) {
    ...
    for (let dy = -radius; dy <= radius; dy++) {
      for (let dx = -radius; dx <= radius; dx++) {
        ...
        if (Math.abs(dist - radius) < 0.7) setPixel(f, center + dy, center + dx, intensity * 0.6);
      }
    }
    frames.push(f);
  }
...
export const Matrix = React.forwardRef<HTMLDivElement, MatrixProps>(
  ({ rows, cols, pattern, frames, fps = 12, autoplay = true, loop = true, size = 10, gap = 2, palette = { on: "#a855f7", off: "#1a1a2e" }, brightness = 1, ariaLabel, onFrame, className, ...props }, ref) => {
    const { frameIndex } = useAnimation(frames, { fps, autoplay: autoplay && !pattern, loop, onFrame });
    ...
    const svgDimensions = useMemo(() => ({ width: cols * (size + gap) - gap, height: rows * (size + gap) - gap }), [rows, cols, size, gap]);

    const isAnimating = !pattern && frames && frames.length > 0;

    return (
      <div ref={ref} role="img" aria-label={ariaLabel ?? "matrix display"} aria-live={isAnimating ? "polite" : undefined} className={cn("relative inline-block", className)} style={{ "--matrix-on": palette.on, "--matrix-off": palette.off } as React.CSSProperties} {...props}>
        <svg width={svgDimensions.width} height={svgDimensions.height} viewBox={`0 0 ${svgDimensions.width} ${svgDimensions.height}`} xmlns="http://www.w3.org/2000/svg" className="block" style={{ overflow: "visible" }}>
          <defs>
          ...
          <style>{`.matrix-pixel { transition: opacity 300ms ease-out, transform 150ms ease-out; transform-origin: center; transform-box: fill-box; } .matrix-pixel-active { filter: url(#matrix-glow); }`}</style>
          {currentFrame.map((row, rowIndex) =>
            row.map((value, colIndex) => {
              ...
              const fill = isOn ? "url(#matrix-pixel-on)" : palette.off;
              const scale = isActive ? 1.1 : 1;
              const radius = (size / 2) * 0.9;
              return (
                <circle key={`${rowIndex}-${colIndex}`} className={cn("matrix-pixel", isActive && "matrix-pixel-active", !isOn && "opacity-20")} cx={pos.x + size / 2} cy={pos.y + size / 2} r={radius} fill={fill} opacity={isOn ? opacity : 0.1} style={{ transform: `scale(${scale})` }} />
              );
            })
          )}
        </svg>
